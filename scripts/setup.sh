...
: "${PIMCORE_RABBITMQ_PASSWORD:?PIMCORE_RABBITMQ_PASSWORD is required}"

urlencode() {
    php -r 'echo rawurlencode($argv[1]);' "$1"
}

db_password_url="$(urlencode "$PIMCORE_DB_PASSWORD")"
opensearch_password_url="$(urlencode "$PIMCORE_OPENSEARCH_PASSWORD")"
rabbitmq_password_url="$(urlencode "$PIMCORE_RABBITMQ_PASSWORD")"

export DATABASE_URL="mysql://pimcore:${db_password_url}@db:3306/pimcore"
export PIMCORE_OPENSEARCH_DSN="opensearch://admin:${opensearch_password_url}@opensearch:9200?ssl=true&ssl_verify=false"
export PIMCORE_MESSENGER_TRANSPORT_DSN_PREFIX="amqp://pimcore:${rabbitmq_password_url}@rabbitmq:5672/%2f/"

app_repo="$(normalize_repo "${PIMCORE_APP_REPOSITORY:-aggrosoft/pimcore-app}")"
clone_repo "$app_repo" "$app_dir"

cat > "$app_dir/.env.dev.local" <<EOF
# Generated by pimcore-dev-runtime. Do not commit.
DATABASE_URL="$DATABASE_URL"
PIMCORE_OPENSEARCH_DSN="$PIMCORE_OPENSEARCH_DSN"
PIMCORE_MESSENGER_TRANSPORT_DSN_PREFIX="$PIMCORE_MESSENGER_TRANSPORT_DSN_PREFIX"
EOF

chown developer:www-data "$app_dir/.env.dev.local"
chmod 0640 "$app_dir/.env.dev.local"
